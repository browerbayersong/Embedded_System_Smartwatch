...
| 总体电路实物 | PCB 终版设计 |
|:---:|:---:|
| ![总体电路](课程报告LaTex模板/figures/总体电路.jpg) | ![PCB终版](课程报告LaTex模板/figures/PCB终版.png) |
...
| 模块 | 说明 | 位置 |
|------|------|------|
| **⭐ 主固件** | Phase 0–5 功能整合版（推荐烧录此版本） | `Embedded_System_Smartwatch/Phase3_Bluetooth/` |
| Phase 0 | HC-05 蓝牙模块 AT 配置工具（首次使用需配置蓝牙参数） | `Embedded_System_Smartwatch/Phase0_HC05Setting/` |
| Phase 1 | OLED 显示驱动（SSD1306 128×64，I2C1） | `Embedded_System_Smartwatch/Phase1_OLED/` |
| Phase 2 | MPU6050 6 轴传感器（加速度 + 陀螺仪 + 姿态角，I2C2） | `Embedded_System_Smartwatch/Phase2_MPU6050/` |
| Phase 3 | 蓝牙通信（USART2 + DMA，115200 baud，HC-05 STATE 引脚检测） | `Embedded_System_Smartwatch/Phase3_Bluetooth/` |
| **Android APP** | 手机端 BLE 上位机（扫描连接、数据显示、时间同步） | `Embedded_System_Smartwatch/Android_app/` |
...
| 完整原理图 | 电源模块（串联3.7V电池） |
|:---:|:---:|
| ![原理图终版](课程报告LaTex模板/figures/原理图终版.png) | ![串联3.7V电池](课程报告LaTex模板/figures/串联3.7V电池.jpg) |
...
| 表盘主页 | 传感器数据 | 蓝牙状态 |
|:---:|:---:|:---:|
| ![手表主页](课程报告LaTex模板/figures/手表主页.jpg) | ![MPU6050](课程报告LaTex模板/figures/手表显示MPU6050数据.jpg) | ![蓝牙](课程报告LaTex模板/figures/手表蓝牙界面.jpg) |

| 设备信息 | 活动计步 |
|:---:|:---:|
| ![设备信息](课程报告LaTex模板/figures/手表设备信息.jpg) | ![计步](课程报告LaTex模板/figures/手表显示步数距离及热量消耗页面.jpg) |
...
| 功能 | 说明 |
|------|------|
| **BLE 设备扫描** | 搜索附近蓝牙设备，显示名称与 MAC 地址 |
...
| 蓝牙扫描 &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 数据接收 & 时间同步 |
|:---:|
| ![安卓上位机扫描设备](课程报告LaTex模板/figures/安卓上位机扫描设备.jpg) |
| ![安卓上位机接收数据](课程报告LaTex模板/figures/安卓上位机接收数据及同步时间.jpg) |

APK 安装包：`Embedded_System_Smartwatch/Android_app/app/release/app-release.apk`（已签名，安装即用）
...
│   ├── Core/
│   │   ├── Inc/
│   │   │   ├── oled.h           # OLED 驱动 API（16×24 大字、绘图）
...
使用 `Embedded_System_Smartwatch/Phase0_HC05Setting/`：

- **方式 A（命令行）**：烧录固件 → USB 串口接 PC → 串口终端发送 `AT+UART=115200,0,0`
- **方式 B（Web 图形界面）**：运行 `hc05_at_host/hc05_at_host.py` → 浏览器打开 Web 界面 → 图形化配置波特率、名称、密码

### 2. 烧录主固件

1. 在 Keil MDK、STM32CubeIDE 或 CLion 中打开 `Phase3_Bluetooth/`
